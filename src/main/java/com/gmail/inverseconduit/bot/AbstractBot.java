 ...
 * {@link AbstractBot#enqueueMessage(ChatMessage) enqueueMessage}.
 * <br /><br />
...
public abstract class AbstractBot implements Subscribable<MessageListener> {
    ...
    /**
     * Maintains the collection of subscribers to the bot. A given subscriber can only exist <b>once</b> in the Map
     */
    ...
    /**
     * Adds a subscriber to be notified of messages processed by this specific Bot
     * 
     * @param subscriber
     *        The subscriber to be notified of messages
     */
    @Override
    public void subscribe(@Nonnull MessageListener subscriber) {
        Objects.nonNull(subscriber);

        ListenerProperty property = subscriber.getClass().getAnnotation(ListenerProperty.class);
        if (null == property) {
            subscribers.put(Priority.DEFAULT, subscriber);
            return;
        }
        ...
        if (time < 0) {
            subscriber = new TimedMessageListener(subscriber, this, time);
        }
    ...
    public void unSubscribe(MessageListener listener) {
        if (!subscribers.values().remove(listener)) {
            ...
            if (null == property) {
                throw new IllegalStateException(); // maybe just return and swallow??
            }
            Priority priority = property.priority();
            int time = property.timer();
            if (time < 0) {
