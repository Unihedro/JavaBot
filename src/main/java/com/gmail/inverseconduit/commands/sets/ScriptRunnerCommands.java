...
public final class ScriptRunnerCommands {
	...
	private static final Pattern evalPattern = Pattern.compile(
			"^" + Pattern.quote(config.getTrigger()) + "eval (.*)",
			Pattern.DOTALL);

	private static final Pattern loadPattern = Pattern.compile(
			"^" + Pattern.quote(config.getTrigger()) + "load (.*)",
			Pattern.DOTALL);
	...
	public static CommandHandle evalCommand(ScriptRunner scriptRunner) {
		return new CommandHandle.Builder(
				...
				message -> {
					...
					matcher.find();
					return scriptRunner.evaluateGroovy(message, matcher
							.group(1).trim());
				})
				.setHelpText(
						"Evaluates a given groovy script. Syntax: '"
								+ config.getTrigger() + "eval {groovy}'")
				.setInfoText("GroovyScript evaluation").build();
	}
